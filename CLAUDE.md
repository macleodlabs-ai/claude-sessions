...
- `claude-config-bootstrap/` — the skill: `SKILL.md` (agent-facing trigger doc), `README.md` (human reference), `scripts/setup.sh` (per-config-dir engine), `scripts/cc-rotate` + `scripts/cc-rotate-kill` + `scripts/cc-pool-sync` (rate-limit rotation supervisor, StopFailure hook helper, and pool parity/relogin sync; installed into `~/.claude-shared/` by the CLI).
- `README.md` — end-user quick start (hero + badges). `LICENSE` — MIT © Macleod Labs. `assets/hero.png` — README banner (generated with Gemini "nano banana"). `docs/relogin.md` — investigation + design notes behind the third-party auth relogin.
...
**Pool parity + relogin (`cc-pool-sync`):** runs full-sync at `--pool-add` and before each rotation, and in `--push-mcp` hook mode from a `SessionEnd` hook on every member. It syncs MCP server definitions (`.claude.json`, jq-merged), plugins/skills/agents/commands (no-ops when symlink-shared), Chrome-extension pairing keys (`chromeExtension`, `cachedChromeExtensionInstalled`, `claudeInChromeDefaultEnabled` in `.claude.json` — secret-free device pairing), and **performs the third-party auth relogin** by cherry-picking exactly three keys from `.credentials.json`: `mcpOAuth` (freshest `expiresAt` wins), `mcpOAuthClientConfig`, `pluginSecrets`. HARD RULE: the Anthropic account keys — `claudeAiOauth`, `trustedDeviceToken`, `designOauth`, `organizationUuid`, `enterpriseGateway` — are never read, written, or copied; credential writes are temp-file + same-dir `mv` at mode 0600, and without `jq` the sync degrades to printing `claude mcp login` instructions (never hand-edits the file). macOS keychain stores are out of reach — skipped with a warning. Full background: `docs/relogin.md`.
