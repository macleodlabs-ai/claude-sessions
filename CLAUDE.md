...
- `claude-session` — **repo-root CLI and primary entry point.** Owns all shell integration. Subcommands: `--create NAME...`, `--delete NAME...`, `--list`, `--pool-add CLIENT`, `--pool-list CLIENT`, `--pool-remove NAME...`, `--revert [--force]`; flags `-y/--yes` (skip prompts), `--force` (with `--revert`, also delete client account dirs incl. pool members). Delegates per-client memory+statusline to `setup.sh`.
