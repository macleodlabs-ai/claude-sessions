...
- `claude-session` — **repo-root CLI and primary entry point.** Owns all shell integration. Subcommands: `--create NAME...`, `--delete NAME...`, `--list`, `--revert [--force]`; flags `-y/--yes` (skip prompts), `--force` (with `--revert`, also delete client account dirs). Delegates per-client memory+statusline to `setup.sh`.
- `claude-config-bootstrap/` — the skill: `SKILL.md` (agent-facing trigger doc), `README.md` (human reference), `scripts/setup.sh` (per-config-dir engine).
